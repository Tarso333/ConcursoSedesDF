...
export function getDueCards(limit: number): DueCard[] {
  return getDb()
    .select({
    ...
    .where(sql`${srsCards.due} <= datetime('now')`)
...
export function getReviewStats(): ReviewStats {
  ...
  const dueNow = db
    ...
    .where(sql`${srsCards.due} <= datetime('now')`)
    .get()
  const total = db.select({ c: count() }).from(srsCards).get()
  const reviewedToday = db
    .select({ c: count() })
    .from(srsReviews)
    .where(sql`date(${srsReviews.review}) = date('now')`)
    .get()
