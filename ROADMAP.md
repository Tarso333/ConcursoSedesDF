...
### M26 — IBGE premium: 100% dos tópicos cobertos ✅ (concluído 2026-07-20)
**Biblioteca premium do IBGE (ACS + ACA), só dados (engines/domínio/migrations intactos).** Passo obrigatório: **diagnóstico por tópico** (script `ibge-diag`, read-only) classificando vazio/parcial/completo — a expansão seguiu exatamente esse diagnóstico (ACS tinha 15 tópicos vazios; ACA, 15). Preenchidos **todos os tópicos e subtópicos-folha vazios** com conteúdo rico estilo apostila (resumo, conceitos, tabelas comparativas, mnemônicos, pegadinhas, palavras-chave): LP (concordância, regência, coesão, redação oficial, sinônimos/homônimos-parônimos, referenciação, conectores, tempos/modos) e RLQ (estruturas lógicas, argumentação, diagramas, aritmética, álgebra/geometria) — compartilhados por slug entre os dois cargos; ACS (folhas de Funções Administrativas: planejamento/organização/direção/controle). Questões IBFC com **comentário alternativa a alternativa** (correta e as quatro incorretas, com fundamento e pegadinha); flashcards variados na fila FSRS; relações do grafo (incl. `REVISAO_RECOMENDADA` do ciclo PODC). **Resultado: ACS 41/41 e ACA 32/32 tópicos com conhecimento (100% de cobertura por tópico em ambos os cargos)**; nenhuma disciplina do IBGE aparece mais no ranking de mais fracas. **Crescimento: +95 conhecimentos, +21 questões, +36 flashcards, +14 relações** (global 673 K · 361 Q · 212 F · 192 R). **Idempotência comprovada** (2º boot → contagens idênticas, zero duplicação).
**Verificado:** 84/84 testes · typecheck + build verdes · boot semeia sem duplicar · demais concursos intactos · fk_check limpo.

### M25 — IBGE completo: cargos ACS + ACA ✅ (concluído 2026-07-20)
**IBGE torna-se concurso completo para dois cargos, só dados (engines/domínio/migrations intactos, sem exceção específica de cargo).** Reanálise integral do Edital 01/2026 (IBFC): **Língua Portuguesa e Raciocínio Lógico Quantitativo são idênticos no Anexo IV para todos os cargos** → autorados uma vez em `ibge/shared.ts` e compartilhados por slug (`lingua-portuguesa`/`raciocinio-logico`) entre os dois contests, com multiConcurso (M16) ativo. (1) **Cargo ACA** cadastrado como contest próprio `ibge-2026-aca` (mesmo padrão ABGF/DATAPREV: um contest por cargo, pois ACA e ACS têm provas diferentes) — LP 15 + RLQ 10 + **Noções de Administração 35** (8 tópicos, 100% cobertos: funções, motivação/comunicação, grupos, delegação, qualidade em serviços, atendimento ao público, documentação/arquivo); R$ 2.128,00, ensino médio; decisões documentadas em `ibge/aca.ts`. (2) **ACS completado**: preenchidos os tópicos que faltavam (Adm: aspectos gerais, gerência/gestão de pessoas; Conhecimentos Técnicos: objeto do Censo, mediação/articulação) + Português/RLQ compartilhados → cobertura **32%→63%** (bloco específico de Conhecimentos Técnicos **100%**). Questões IBFC com justificativa alternativa a alternativa; flashcards na fila FSRS; relações do grafo por cargo. **Crescimento: +98 conhecimentos, +27 questões, +45 flashcards, +9 relações** (global 578 K · 340 Q · 176 F · 178 R). Decisão de modelagem: conhecimento anexado ao tópico-pai (subtópicos-folha sem duplicação). 5 concursos ativos.
**Verificado:** 84/84 testes · typecheck + build verdes · boot semeia sem duplicar · SEDES/ABGF/DATAPREV/IBGE-ACS intactos · fk_check limpo.
